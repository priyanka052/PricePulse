...
def send_alert(previous: int, latest: int):
    """
    Send a Telegram message when a price drop is detected.
    Shows previous price, new price, and amount saved.
    """
    try:
        if BOT_TOKEN == "YOUR_BOT_TOKEN_HERE" or CHAT_ID == "YOUR_CHAT_ID_HERE":
            print(
                ...
                f"(Was: Rs.{previous} → Now: Rs.{latest})"
            )
            return False
        ...
        print(f"[notifier] Alert sent successfully (saved Rs.{saved}).")
        return True
...
if __name__ == "__main__":
    ...
    send_alert(previous=59999, latest=54999)
